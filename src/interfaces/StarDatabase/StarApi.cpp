                                     ...
                                     const double& latitude, const double& longitude, const double& elevation)
{
   ...
   const std::time_t t1{std::chrono::system_clock::to_time_t(startMinute)};
   ...
   const std::time_t t2{std::chrono::system_clock::to_time_t(endMinute)};
   ...
   myEasyCurlRequest.setOpt(curlpp::Options::Url(queryUrl));
   ...
   catch(const std::exception& e)
   {
      ...
      LOG_ERROR("Failed to perform curl request: " + e.what());
      return ApiResponse(ApiResponseEnum::FAILURE);
   }
...
{
   const auto encodedParam{curl_easy_escape(myEasyCurlRequest.getHandle(), param->c_str(), param->size())};
...
{  
   ...
   {
      LOG_ERROR("Invalid response format: " + line);
      return ApiResponse(ApiResponseEnum::FAILURE); // FAILURE
   }
   std::getline(response, line);
   ...
   if (successCase)
   {
      while(std::getline(response, line))
      {
         ...
         result.myResponseResults.push_back(line);
      }
      result.myType = ApiResponseEnum::SUCCESS;
   }
   else if (optionsCase)
   {
      while(std::getline(response, line))
      ...
      result.myType = ApiResponseEnum::USER_OPTIONS;
   }
   else if (noMatchCase)
   {
      result.myType = ApiResponseEnum::NO_MATCH;
   }
